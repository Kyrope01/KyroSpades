...
void config_save() {
	...
	config_seti("client", "player_count", settings.player_count);
...
static int config_read_key(void* user, const char* section, const char* name, const char* value) {
	if(!strcmp(section, "client")) {
		...
		IMPORT_SETTING(settings.player_count, player_count, atoi(value));
...
void config_reload() {
	...
	list_add(&config_settings,
			 &(struct config_setting) {
				 .value = &settings_tmp.player_count,
				 .type = CONFIG_TYPE_INT,
				 .min = 0,
				 .max = 1,
				 .help = "Displays player count in both teams at the bottom of the screen",
				 .name = "Player Count",
			 });
