...
void chunk_draw_visible() {
        ...
        // never being drawn.  Size the array from the actual iteration count.
        int iter = CHUNKS_PER_DIM + 2 * overshoot;
        int cap = iter * iter;
        struct chunk_render_call* chunks_draw = malloc(sizeof(struct chunk_render_call) * (size_t)cap);
        if(!chunks_draw) {
