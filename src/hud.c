...
static void hud_ingame_render(mu_Context* ctx, float scalex, float scalef) {
	...
	if(settings.player_count && !network_map_transfer && !(window_key_down(WINDOW_KEY_HIDEHUD))) {
		int count_team1 = 0;
		int count_team2 = 0;
		...
		float y_offset = 25.F;
		float y = y_offset + font_size;

		char count_str[8];
		sprintf(count_str, "%i", count_team1);
		glColor3ub(gamestate.team_1.red, gamestate.team_1.green, gamestate.team_1.blue);
		font_centered((settings.window_width - spacing) / 2.F, y, font_size, count_str);

		sprintf(count_str, "%i", count_team2);
